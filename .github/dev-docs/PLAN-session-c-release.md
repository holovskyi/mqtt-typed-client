...
**Гілка:** `main`. Версії незалежні: **root/core/macros = 0.2.0**, **engine/doc-macros = 0.1.0**
...
Залежності (path+version): `doc-macros` ← `engine` ← `core` ← `macros`; root ← core+macros+doc-macros.
Тому публікувати **строго**:

1. `mqtt-typed-client-doc-macros` (0.1.0) — лист, без внутрішніх залежностей
2. `mqtt-topic-engine` (0.1.0) — залежить від doc-macros
3. `mqtt-typed-client-core` (0.2.0) — залежить від engine
4. `mqtt-typed-client-macros` (0.2.0) — залежить від core
5. `mqtt-typed-client` (0.2.0, root) — залежить від core+macros+doc-macros
...
- **doc-macros** — повний `--dry-run` працює (лист).
...
- **Перевірити доступність НОВИХ імен на crates.io:** `mqtt-topic-engine` та
  `mqtt-typed-client-doc-macros` — чи вільні / чи нашого власника. (root/core/macros вже наші з 0.1.0.)
...
- Версії узгоджені (перевірено: root/core/macros=0.2.0, engine/doc-macros=0.1.0; install-сніпети
  root README=0.2.0, engine README=0.1.0). Усі `version=` у path-deps звірені дослівно
  (engine→doc-macros=0.1.0; core→engine=0.1.0; macros→core=0.2.0; root→core/macros=0.2.0).
...
- **`publish = false` має бути ВІДСУТНІЙ** у всіх 5 (перевірено — його ніде немає; усі 5 публікуються).
...
`doc-macros/CHANGELOG.md` (`[0.1.0]`). `[Unreleased]`-секції лишити порожніми (стандарт).
...
**🔴 ОБОВ'ЯЗКОВО ДО будь-якої публікації — для ВСІХ 5 крейтів** (зловити проблеми пакування/
...
```
Повний `cargo publish --dry-run --locked` тут пройде лише для `doc-macros` (лист); для решти
...
Для кожного крейта в порядку 1→5:
```
...
```
Послідовність:
1. `cargo publish -p mqtt-typed-client-doc-macros`
2. `cargo publish -p mqtt-topic-engine`
3. `cargo publish -p mqtt-typed-client-core`
4. `cargo publish -p mqtt-typed-client-macros`
5. `cargo publish -p mqtt-typed-client`
...
- **Часткове падіння (напр. 3 з 5 опубліковано):** опубліковані крейти НЕЗВОРОТНІ. Якщо після цього
  ...
  верифікація + package/build усіх 5) — обов'язковий ДО першого publish.
- **Compare-лінки CHANGELOG:** тег один — `v0.2.0`. Перевірити, що engine/doc-macros CHANGELOG
  (які виходять 0.1.0) НЕ містять битих `compare/v0.1.0...`-лінків на неіснуючі теги (наразі не містять).
...
- **Path+version неузгодженість** — кожна `version` у path-deps має точно відповідати опублікованій
  (doc-macros/engine = 0.1.0; core = 0.2.0). Перевірено, але звірити ще раз у Кроці 0.
