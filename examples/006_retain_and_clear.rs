...
use mqtt_typed_client::{BincodeSerializer, MqttClient, MqttClientError, MqttConnection};
...
async fn create_client(client_id: &str) -> Result<(MqttClient<BincodeSerializer>, MqttConnection), MqttClientError> {
...
async fn run_delayed_subscriber(delay_seconds: u64, client_id: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let now = chrono::Utc::now().format("%H:%M:%S%.3f").to_string();
    println!("[{}] {}: Connected at t={}s, checking for retained messages...", 
        client_id.to_uppercase(), now, delay_seconds);
    ...
    match timeout_result {
        ...
        Ok(Some(Ok(msg))) => {
            let now = chrono::Utc::now().format("%H:%M:%S%.3f").to_string();
            println!("[{}] {}: ✓ Received retained message: '{}'", 
                client_id.to_uppercase(), now, msg.payload.content);
        }
        ...
        Ok(None) | Err(_) => {
            let now = chrono::Utc::now().format("%H:%M:%S%.3f").to_string();
            println!("[{}] {}: ✗ No retained message received (broker storage empty)", 
                client_id.to_uppercase(), now);
        }
    }
