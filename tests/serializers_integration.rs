...
mod serializer_tests {
    ...
    async fn test_messagepack_serializer() {
        test_serializer_integration::<MessagePackSerializer>("MessagePack").await;
    }
    ...
    async fn test_flexbuffers_serializer() {
        test_serializer_integration::<FlexbuffersSerializer>("Flexbuffers").await;
    }
...
{
    let url = format!("mqtt://localhost:1883?client_id=test_{}_integration", name.to_lowercase());
    ...
    match MqttClient::<S>::connect(&url).await {
        Ok((client, connection)) => {
            ...
            assert!(publisher.is_ok(), "{} serializer: Failed to create publisher", name);
            
            // If we have both publisher and can create subscriber, test full cycle
            match client.subscribe::<TestMessage>(topic.as_str()).await {
                Ok(mut subscriber) => {
                    println!("{} serializer: Testing full publish/subscribe cycle", name);
                    ...
                    if let Ok(pub_result) = publisher.unwrap().publish(&message).await {
                        println!("{} serializer: Message published successfully", name);
                        
                        // Try to receive with timeout
                        let timeout = tokio::time::Duration::from_millis(1000);
                        let receive_result = tokio::time::timeout(timeout, subscriber.receive()).await;
                                ...
                                match result {
                                    Ok(received_message) => {
                                        assert_eq!(received_message, message, 
                                            "{} serializer: Message mismatch", name);
                                        println!("{} serializer: Full cycle successful (serialize + deserialize)", name);
                                    }
                                    Err(e) => {
                                        panic!("{} serializer: Deserialization failed: {:?}", name, e);
                                    }
                                }
                            }
                            Ok(None) => {
                                println!("{} serializer: ⚠️ No message received (broker might be busy)", name);
                            }
                            Err(_) => {
                                println!("{} serializer: ⚠️ Receive timeout (broker might be slow)", name);
                            }
                        }
                    } else {
                        println!("{} serializer: ⚠️ Publish failed (broker might be busy)", name);
                    }
                }
                Err(e) => {
                    println!("{} serializer: ⚠️ Subscription failed: {:?}", name, e);
                }
            }
            
            // Always shutdown gracefully
            let shutdown_result = connection.shutdown().await;
            assert!(shutdown_result.is_ok(), "{} serializer: Failed to shutdown", name);
        ...
        Err(e) => {
            ...
            println!("{} serializer: Connection failed (expected in CI without broker): {:?}", name, e);
            
            // Even without broker, we can test that the serializer compiles and can be instantiated
            let _serializer = S::default();
            println!("{} serializer: Serializer instantiation successful", name);
        }
    }
}
...
{
    let url = format!("mqtt://localhost:1883?client_id=test_{}_connection", name.to_lowercase().replace(" ", "_"));
    ...
    match MqttClient::<S>::connect(&url).await {
        Ok((client, connection)) => {
            ...
            println!("{} serializer: Connection test successful (messaging requires generated types)", name);
        }
        Err(e) => {
            // Connection failed - this is OK in CI/test environments without MQTT broker
            println!("{} serializer: Connection failed (expected in CI without broker): {:?}", name, e);
